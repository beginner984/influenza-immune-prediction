# R/08_evaluate_random_forest_cv.R
# Evaluate Random Forest using cross-validated training predictions only.
...
model_file <- "models/random_forest_model.rds"
result_dir <- "results"
...
random_forest_model <- readRDS(model_file)

# caret saved out-of-fold predictions for the best tuning values.
cv_predictions <- random_forest_model$pred

# ------------------------------------------------------------
# 3. Confirm required prediction columns exist
# ------------------------------------------------------------
...
# Each participant was predicted once in every repeat.
# Average the repeated out-of-fold probabilities to produce
...
# Higher probabilities should indicate Positive cases.
roc_object <- roc(
...
# ------------------------------------------------------------
# 5. Select a threshold from training data only
# ------------------------------------------------------------
...
write.csv(
  cv_averaged,
  file.path(result_dir, "random_forest_cv_predictions.csv"),
  row.names = FALSE
)

write.csv(
  performance_table,
  file.path(result_dir, "random_forest_cv_performance.csv"),
  row.names = FALSE
)
...
ggsave(
  filename = file.path(result_dir, "random_forest_cv_roc.png"),
  plot = roc_plot,
  width = 6,
  height = 5,
  dpi = 300
...
cat("\nRandom Forest cross-validation evaluation completed.\n\n")
